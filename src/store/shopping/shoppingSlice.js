    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getBooksForCart.fulfilled, (state, action) => {
                ...
                state.totalPrice = state.booksInCart.reduce((total, book) => total + book.price, 0); // Itera sobre los libros en el carrito para mantener actualizado el subtotal
            })
            ...
            .addCase(removeBookFromCart.fulfilled, (state, action) => {
                const bookToRemove = state.booksInCart.find(book => book.id === action.payload.id);
                if (bookToRemove) {
                    state.totalPrice -= bookToRemove.price;
                    state.booksInCart = state.booksInCart.filter(book => book.id !== action.payload.id);
                }
            });
    },
});
